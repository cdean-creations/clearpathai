...
const { sendCallSummaryEmail } = require("./email");
    ...
    try {
      if (name === "save_lead") {
        ...
        results.push({
          ...
          result: `Lead saved for ${lead.name}. Details will be emailed to the business when the call ends.`,
        });
        ...
        results.push({
          toolCallId: toolCall.id,
          result: "Callback request logged. The business will get the full call summary by email when the call ends.",
        });
      } else {
        results.push({
          toolCallId: toolCall.id,
...
async function handleEndOfCallReport(message, business) {
  ...
  try {
    await sendCallSummaryEmail(
      {
        ...
        phone: existingLead?.phone || report.callerPhone || "",
        address: existingLead?.address || "",
        issue: existingLead?.issueType || "",
    ...
    console.error("[vapi] call summary email failed:", err.message);
  }
