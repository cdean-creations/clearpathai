...
function buildLeadHtml({ name, phone, address, issue }) {
  return `
    <h2 style="font-family:sans-serif;margin:0 0 12px">New call lead</h2>
    <table style="border-collapse:collapse;font-family:sans-serif;font-size:15px;line-height:1.4">
      <tr>
...
/**
 * Fallback email if a call ended without a mid-call save_lead.
 * Same customer-facing fields only — no duration, cost, or ended reason.
 */
async function sendCallSummaryEmail(report, business) {
  ...
  const html = buildLeadHtml({
    ...
    issue: report.issue || report.summary,
  });

  if (!resend) {
    console.log("[email] Resend not configured — call lead email skipped:", subject);
    return { skipped: true, subject };
  }
  ...
  if (error) {
    console.error("[email] Failed to send call lead email:", error);
    throw new Error(typeof error === "string" ? error : error.message || "Email failed");
  }
