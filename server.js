...
<body style="font-family:Arial,sans-serif;background:#f5f5f5;padding:20px">
  <div style="background:white;border-radius:8px;padding:24px;max-width:600px;margin:auto;border:1px solid #ddd">
    ...
    <p style="display:inline-block;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:bold;
      background:${escalated ? "#fff0f0" : "#f0fff4"};
      color:${escalated ? "#c0392b" : "#1a7a3a"};
      border:1px solid ${escalated ? "#e74c3c" : "#27ae60"}">
...
app.post("/webhook/vapi", async (req, res) => {
  ...
  try {
    await transporter.sendMail({
      from: `ClearPath AI <${CONFIG.fromEmail}>`,
      to: CONFIG.demoEmail,
      subject,
